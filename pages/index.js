...
import { SITENAME, faqs } from "@/data";
...
export default function Home() {
  ...
  //   Image width={300} height={300}Url: "//a.impactradius-go.com/display-ad/5618-1130495",
  ...
  const handleDownloadMP4 = (url) => {
    ...
    try {
      FileSaver.saveAs(url, { SITENAME } + new Date().getTime() + ".mp4");
    } catch (error) {
  ...
  const handleDownloadMP3 = (url) => {
    try {
      FileSaver.saveAs(url, { SITENAME } + new Date().getTime() + ".mp3");
    } catch (error) {
      alert(error.message);
    }
  };
    ...
    <main className="w-full max-w-5xl mx-auto px-3">
      ...
      {/* <section className="w-full py-8 flex flex-col items-center justify-center bg-slate-800 px-4">
        <h3 className="text-white text-xl sm:text-2xl font-semibold mb-2 text-center">
          Download {SITENAME} On Your Android Devices For Free
        </h3>
        <span className="text-slate-200 text-sm sm:text-base text-center mb-5">
          Get easier access to {SITENAME} by downloading our app
        </span>
      ...
      <section id="howtouse" className="w-full py-10 sm:py-16">
        <div className="w-full text-center flex flex-col gap-2 mb-8">
          ...
          <span className="text-slate-700 text-sm sm:text-base">
            Discover how to use {SITENAME} to enhance your experience
          </span>
        </div>
        <div className="flex flex-col gap-12 w-full">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-20">
            <div className="flex flex-col gap-3 order-1 sm:order-none">
              <h3 className="text-xl sm:text-3xl font-bold text-center sm:text-start text-slate-800">
                2. Visit the {SITENAME} Website
              </h3>
              <p className="text-slate-700 text-center sm:text-start">
                Launch browser, enter www.{SITENAME}.app in address bar, visit{" "}
                {SITENAME}'s site on computer or mobile.
              </p>
            </div>
            <Image
              width={300}
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-20">
            <div className="flex flex-col gap-3 order-1 sm:order-none">
              ...
              <p className="text-slate-700 text-center sm:text-start">
                {SITENAME} will process the URL, creating download link.
                Click/tap "Download MP4," choose save location. After download,
                open file for watermark-free TikTok video enjoyment!
              </p>
            </div>
            <Image
              width={300}
